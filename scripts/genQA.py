...
限制条件：a.问题内容需要聚焦于知识本身而非其参考的表格，图片等。不要问出"表xx中可以看出什么?"，"相关信息可以在表几找到?"这种具体指代某个表格或图片的问题。b.不要直接对某一章节的索引问问题，例如"第几章的内容是什么？","第四章和第三章是否独立"等。
考点知识：{}
输出格式：
请返回json格式,例如:
{{
    "questions":[
        {{
            ...
            "question": "在多盘摩擦离合器中，如果摩擦系数f增加，而其他条件不变，那么可传递的最大转矩T_max会成比例增加", 
            "answer": "正确", 
            "analysis": "根据公式T_max = fFR，可以看出最大转矩T_max与摩擦系数f成正比。因此，在其他条件（F和R）不变的情况下，摩擦系数f增加，最大转矩T_max也会成比例增加。"
        }},
        {{
            "task": "Reasoning",
            ...
            "answer": "100 N·m", 
            "analysis": "根据公式T_max = fFR，代入已知数值：T_max = 0.5 * 1000 N * 0.2 m = 100 N·m。因此，该离合器可传递的最大转矩为100 N·m。"
        }},
        {{
            "task": "Reasoning", 
